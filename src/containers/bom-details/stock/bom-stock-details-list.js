...
export default function BomStockDetailsList({element, index, stockDetails, setStockDetails, removeStockDetails}) {
    ...
    const changeInOut = (item) => {
        setInOut(item);
        setInOutRenders((prevValue) => prevValue + 1);
        setInOutDD(false);
    }
    ...
    const handleStockDetails = () => {
        ...
        tempProcs[index]["bom_quantity"] = quantity;
        tempProcs[index]["in_out"] = inOut;
        setStockDetails(tempProcs);
    }
    useEffect(() => {
        handleStockDetails();
    }, [stock, quantity, inOut]);    
    ...
    return (
        <Card.InputColumn center notResponsive>
            ...
            <Card.MultiInputContainer>
                <Card.MultiInput type="number" id="quantity" placeholder=" " autoComplete="off" onKeyDown={blockInvalidNumber} value={element?.bom_quantity || ''} onChange={({target}) => handleQuantity(target.value)} />
            </Card.MultiInputContainer>
            <Card.ButtonContainer width="fit-content">
                <Card.AddButton danger deletes small onClick={() => removeStockDetails(index)} />
