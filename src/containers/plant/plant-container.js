...
export default function PlantContainer() {
    ...
    const {JWT} = useAuth();
    const {plantsGridDefn} = useGridColumnDefns();
    ...
    const [columnDefs, setColumnDefs] = useState(plantsGridDefn);
    ...
    const toggleViewGrid = async () => {
        try {
            const response = await axiosPrivate.get("/plant/get-all-plant");
            if(response?.data) {
            ...
            navigate("/plant", { state: { from: location }, replace: true });
        }
        finally {
            setViewGrid(true);
            setAddPlant(false);
    ...
    const handleAddPlant = () => {
        axiosPrivate
        .post("/plant/create-plant", {},{params: {plant_name: name, plant_code: code, company_id: company?.id, branch_id: branch?.id, created_by: JWT?.user?.username, created_on: getNow()}})
        .then(function (response) {
    ...
    const handleDeletePlant = () => {
        axiosPrivate
        .delete("/plant/delete-plant/"+rowSelected.id)
        .then(function (response) {
            setAlert({msg: `Success: ${response?.data[0]}`, type: "success"});
        })
        .catch(function (error) {
    ...
    return (
        ...
        <Card>
            ...
            <Card.ButtonGroup flexStart>
                ...
                <Button small onClick={() => toggleAddPlant()}>Add plant</Button>
            </Card.ButtonGroup>
        </Card>
        {addPlant &&
            <Card width="75%">
                ...
                <Card.ButtonGroup marginTop>
                    ...
                    <Button disabled={isInvalid()} onClick={() => handleAddPlant()} type="submit">Add plant</Button>
                </Card.ButtonGroup>
            </Card>
        }
        {viewGrid &&
            <Grid>
                ...
                {rowSelected.id && <Grid.Text>Selected: {rowSelected.plant_name}</Grid.Text>}
                {/* {rowSelected.id && */}
                    <Grid.ButtonGroup flexStart>
                        <Button disabled={operationInvalid()} small onClick={() => navigate(`/plant/${rowSelected.id}/view`)}>View</Button>
                        <Button disabled={operationInvalid()} small onClick={() => navigate(`/plant/${rowSelected.id}/update`)}>Update</Button>
        ...
        {(deleteModal) ? 
            <DeleteModalContainer deleteFunction={handleDeletePlant} setDeleteModal={setDeleteModal}>Are you sure you want to delete this plant?</DeleteModalContainer>
        : null}
        </>
    );
}